...
//	gPrior denotes the probability of being DE, and it follows: gPrior ~ Beta(0.5,0.5), that is, the Jefreys Prior for a Bernoulli trial
	...
	double minuslog2, logPriorConstant;
...
//****************************************************************************************************************************************
	int n_obs, c_sum, c_sum_new, c_sum_old;								// number of components and number of observations
//****************************************************************************************************************************************
...
int main()
{
	...
	minuslog2 = - log(2.0);
	logPriorConstant = - log(2.0) - log( pow(2.0, K + 0.0) - K - 1.0);
...
//*******************************************************************************************
	...
	double geneDEprobability = 0.0, geneDEprobChain;
	int  initial_state;
...
//	ofstream gThan ( xgty_file.data() );
	...
	*/
		...
		int sample1, sample2;
		double cum_sum1, cum_sum2, tot_sum1, tot_sum2, uni;
	/*			Burn-in period
	##################################################################################################################################
	*/
...
//		cout << endl;
	...
	*/
		...
		vector<double> cConditionalDist (4,0.0);
		vector<double> normalisedDist (4,0.0);
		int c_sum_restricted;
		tot_sum1 = r[0] + s[0] + K;
		tot_sum2 = r[0] + K;
	...
	//		cout << "............ iter = "<< iter << "..............." <<endl;
			...
			// componentsIndex[0] and componentsIndex[1] correspond to the randomly selected pair of components.
			c_sum_restricted = c_sum - c_vec[componentsIndex[0]] - c_vec[componentsIndex[1]];
			// to c_new to exw arxikopoihsei sto permutation_current_collapsed.
			if( c_sum_restricted == 0 ){
				// candidate states for selected components: (1,1), (0,0)
				...
				cConditionalDist[1] = log_f_c_new() + minuslog2;
				uni = unifRand();
				normalisedDist[0] = 1.0/(1.0 + exp(cConditionalDist[1] - cConditionalDist[0]));
				//cout << cDist[0] << endl;
				if(uni < normalisedDist[0]){
					c_vec[componentsIndex[0]] = 1;
					c_vec[componentsIndex[1]] = 1;
				}else{
					c_vec[componentsIndex[0]] = 0;
					c_vec[componentsIndex[1]] = 0;
				}

				
			}else{
				if( c_sum_restricted > 1 ){
					// candidate states for selected components: (1,1), (0,1), (1,0), (0,0)
					// upologizw to cConditionalDist gia to (1,1) (thesi 0)
					c_new[componentsIndex[0]] = 1;
					c_new[componentsIndex[1]] = 1;
					cConditionalDist[0] = log_f_c_new() + logPriorConstant;
					...
					normalisedDist[2] = 1.0/(1.0 + exp(cConditionalDist[0] - cConditionalDist[2]) + exp(cConditionalDist[1] - cConditionalDist[2]) + exp(cConditionalDist[3] - cConditionalDist[2]));
					if(uni < normalisedDist[0]){
						c_vec[componentsIndex[0]] = 1;
						c_vec[componentsIndex[1]] = 1;
					}else{
						if(uni < normalisedDist[0] + normalisedDist[1]){
							c_vec[componentsIndex[0]] = 0;
							c_vec[componentsIndex[1]] = 1;
						}else{
							if(uni < normalisedDist[0] + normalisedDist[1] + normalisedDist[2]){
								c_vec[componentsIndex[0]] = 1;
								c_vec[componentsIndex[1]] = 0;
							}else{
								c_vec[componentsIndex[0]] = 0;
				...
				}else{// c_sum_restricted = 1
					// candidate states for selected components: (1,1), (0,1), (1,0)
					// upologizw to cConditionalDist gia to (1,1) (thesi 0)
					c_new[componentsIndex[0]] = 1;
					c_new[componentsIndex[1]] = 1;
					cConditionalDist[0] = log_f_c_new() + logPriorConstant;
					...
					normalisedDist[1] = 1.0/(1.0 + exp(cConditionalDist[0] - cConditionalDist[1]) + exp(cConditionalDist[2] - cConditionalDist[1]));
					if(uni < normalisedDist[0]){
						c_vec[componentsIndex[0]] = 1;
						c_vec[componentsIndex[1]] = 1;
					}else{
...
//			if (c_sum == 1){
				// prepei na kratisw to proigoumeno c
//				cout << "not permitted c" << endl;
//				for(k = 0; k < K; k++){
//					c_vec[k] = c_old[k];
//				}
//				c_sum = c_sum_old;
...
//###########################################################################################
				...
				if (iter % 1000 == 0){
					t_end = omp_get_wtime( );
					cout << "Iteration " << iter <<": gene DE rate = "<<setprecision(3)<<100.0*geneDEprobChain/nIterations <<"%, time: "<< setprecision(5)<< t_end - t_start<<" sec."<< "\n\n";
					cout << sum_1[0] << ", " << sum_2[0]<<"\n";
				}
			}
	//##################################################################################################################
		}
		geneDEprobability += geneDEprobChain/(nIterations/(chain + 1.0) + 0.0);
		ar_out << geneDEprobChain/(nIterations/(chain + 1.0) + 0.0) << endl;
	}
	geneDEprobability = geneDEprobability/(numberChains + 0.0);
	ar_out << geneDEprobability << endl;
...
for (k = 0; k<K; k++){
	...
	componentsIndex[k] = k; // auto
	c_new[k] = c_vec[k]; // kai auto ta vazw edw gia na apofeugw alla loop ston sampler. Den xreiazontai gia to permutation.
}
}
